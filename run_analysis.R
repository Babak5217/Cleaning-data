...
## merge x_test and y_test
                testbind<-cbind(X_test,y_test)
                trainbind<- cbind(X_train,y_train)
...
##calculated means of data for each type of activitiis
                 ...
                 Finallist<- data.frame( sapply(dataspilit$LAYING,mean), sapply(dataspilit$SITTING,mean),
                                         sapply(dataspilit$STANDING,mean), sapply(dataspilit$WALKING,mean),
                                         sapply(dataspilit$WALKING_DOWNSTAIRS,mean), sapply(dataspilit$WALKING_UPSTAIRS,mean))
                 names(Finallist) = activity_labels$V2
